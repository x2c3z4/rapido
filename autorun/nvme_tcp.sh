...
#echo "1" >/sys/module/nvme_tcp/parameters/cpu_affinity_list
#cat /sys/module/nvme_tcp/parameters/cpu_affinity_list
...
if (( $kcli_rapido_vm_num == 1 )); then
	...
	echo tcp > ${nvmet_cfs}/ports/1/addr_trtype || _fatal
...
if (( $kcli_rapido_vm_num == 2 )); then
	nvme connect -t tcp -a "$vm1_ip" -s 4420 -n nvmf-test -nr-poll-queues 1 --nr-write-queues 2 --nr-io-queues 4 || _fatal
	udevadm settle
